...
subtitle: "`r emo::ji('cake')` start with cake! <br> `r emo::ji('link')` [bit.ly/design-ds-eku-web](http://bit.ly/design-ds-eku-web)"
author: "<br> dr. mine çetinkaya-rundel"
date: "2 april 2021"
output:
...
```
.small[
```{r unvotes-multivariate, echo=FALSE, out.width="100%", fig.width = 9, fig.asp = 1}
un_votes %>%
  filter(country %in% c("United States")) %>%
  inner_join(un_roll_calls, by = "rcid") %>%
  inner_join(un_roll_call_issues, by = "rcid") %>%
  mutate(importantvote = ifelse(importantvote == 0, "No", "Yes")) %>%
...
.discussion[
The following is used to create the multivariate visualisation from earlier. How much of the code would you show/hide when just starting teaching ggplot2?
]
...
.pull-right[
```{r ref.label="scaffold-layer-1", echo=FALSE, out.width = "100%"}
```
]
...
.pull-right[
```{r ref.label="scaffold-layer-2", echo=FALSE, out.width = "100%"}
```
]
...
.pull-right[
```{r ref.label="scaffold-layer-3", echo=FALSE, out.width = "100%"}
```
]
...
.pull-right[
```{r ref.label="scaffold-layer-4", echo=FALSE, out.width = "100%", fig.asp = 1.3}
```
]
...
.pull-left[
.small[
```{r scaffold-layer-5, eval=FALSE, out.width = "100%"}
ggplot(data = us_votes, 
  mapping = aes(x = importantvote, 
                fill = vote)) +
  geom_bar(position = "fill") +
...
.pull-right[
```{r ref.label="scaffold-layer-5", echo=FALSE, out.width = "100%", fig.asp = 1.3}
```
]
...
.pull-right[
```{r ref.label="scaffold-layer-6", echo=FALSE, out.width = "100%", fig.asp = 1.3}
```
]
...
.pull-right[
```{r ref.label="scaffold-layer-7", echo=FALSE, out.width = "100%", fig.asp = 1.3}
```
]
...
.small[
- Your challenge is to go from nothing to a data visualisation in 15 minutes of your first class. Don't worry about the computing infrastructure, assume students have successfully landed in RStudio Cloud like you did earlier. Design an exercise for them to "create" their first visualisation.
- Need inspiration? Use `ggplot2::diamonds`, `dplyr::starwars`, dataset from `nycflights13` or `gapminder` packages.
- If you already have a first day exercise you like, you can modify it to fit the challenge: from nothing to a data visualisation in 15 minutes!
- Compare notes with breakout group mates. 
- Then, take one minute to write down one concrete change you'd like to consider making in your teaching based on what we've covered in this module.
]]
